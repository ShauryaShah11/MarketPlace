...
const Category = () => {
   ...
   useEffect(() => {
      const fetchData = async () => {
         try {
            const order = await handleAllCategory();
            setcust(order);
         } catch (error) {
   ...
   return (
      <>
         ...
         {cust && (
            <div className="ml-64" style={{backgroundColor:"#FFFAFA"}}>
               ...
               <div style={{ height: '100%' }}>
                  {cust?.data?.map((item, key) => (
                     ...
                     >
                        ...
                        >
                           <span style={{ marginLeft: '350px' }}>Category no. :- {item?._id}</span>
                        </Card.Header>
                        <Card.Body style={{ display: 'flex', flexDirection: 'row', height: '120px', marginLeft: '10px' }}>
                           <div style={{ flex: 1, marginTop: '10px' }}>
                              <Card.Text style={{ color: '#000', fontSize: '1rem' }}>
                                 <span style={{ fontStyle: 'italic', marginTop: '20px' }}>Category name : {item?.name} </span>
                              </Card.Text>
                              <Card.Text style={{ color: '#000', fontSize: '1rem' }}>
                                 <span style={{ fontStyle: 'italic', marginTop: '10px' }}>Category Description :{item?.description}</span>
                              </Card.Text>
               ...
               </div>
               {
                ...
                <div className="fixed top-0 left-0 w-full h-full bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
                    <div className="bg-white p-6 rounded-lg shadow-xl" style={{width:"400px",height:"520px"}}>
                        <h2 className="text-xl font-bold mb-4" >Add Category</h2>
                        <AddCat /> {/* Assuming CategoryForm is a component for adding categories */}
                        <button
                                    style={{
